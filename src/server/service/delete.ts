...
export async function deleteTags(userId: string, tagNames: string[]) {
  try {
    if (userId==null || tagNames.length == 0) throw new Error("Invalid option data");
...
export async function deleteTagConnectionsByName(userId: string, diaryId: string, tagNames: string[]) {
  try {
    if (userId==null || diaryId==null || tagNames.length == 0) throw new Error("Invalid option data");
    const deleted: z.infer<typeof tagsSchema>[] = [];
    for(const name of tagNames) {
      const tag = await getTagByName(userId, name);
      if(tag==null) throw new Error("err in getTagByName");
      const deletedTag = await deleteTagConnection(diaryId, tag.id as unknown as string);
