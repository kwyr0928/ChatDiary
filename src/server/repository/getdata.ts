import { type Diaries } from "@prisma/client";
import { z } from "zod";
import {
  analysesSchema,
  chatsSchema,
...
export const getDateDiariesByUserId = async (
  userId: string,
  start: Date,
  end: Date,
) => {
...
export const getTagConnectionsByTag = async (
  diaryId: string,
  tagId: string,
) => {
  try {
...
export const getMonthlyFeedBack = async (userId: string, month: number) => {
  try {
    const data = await db.monthlySummaries.findFirst({
      where: { userId, month },
    });
    if (data == null) return null;
